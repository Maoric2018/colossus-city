...
Powered soaring also breaches buildings. `shared/soar-breach.js` sweeps a 2.8 m half-width/height pressure field ahead of the prone pilot, with up to 6.5 m of extra lead plus the current tick's travel and 1.3 m forward extent. The shared query selects at most 24 intersected cells per tick. Only fixed props or existing rubble actually in the pilot's capsule path stop it; ground grazing the wider field does not. Server fracture uses this exact volume to remove individual pieces. Per-fragment outward velocities clear the corridor, including any remaining pieces released when a bay loses all support. Real rubble persists and normal surrounding structural cohesion remains active. Only the responsible pilot gets 0.9 seconds of contact filtering for those cells.

Prediction uses the same field selection and oriented capsule bounds while the server retains destruction authority. It keeps terrain/prop collision. Forward lead gives wall-fracture events time to arrive before the camera reaches the wall. Stale input, normal flight and normal dodges cannot activate a breach; soaring remains available with an empty fuel meter. Breach events add a restrained sound, edge pulse and 0.035 camera shake.

Soar VFX use the original CodeManu hyperspeed sheet and morningkingdom smoke-shockwave sheet in `public/assets/imported/flight-vfx/`; source pages, frame layouts and hashes are recorded in `SOURCES.json`. The 30-frame speed animation maps around the screen edges with frame interpolation and a clear center. The 68-frame smoke animation plays at entry, with nearby world bursts fading before they overwhelm the viewer. Both sheets share cached textures; up to eight world bursts use one instanced draw and the desktop overlay uses one draw in the existing renderer. The overlay therefore reaches both WebRTC and fallback spectator captures without a second canvas draw. It is disabled for the headset and inactive/dead pilots; world bursts remain visible in stereo.
