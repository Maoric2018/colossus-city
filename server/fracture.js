...
import {fractureRecipe,pieceAlive,pieceDistance,pieceEnergy,fractureStrength,shardBallistic} from '../shared/city/fracture.js';
...
export function initFracture(room){room.shards=new Map();room.activeShards=new Set();room.ballisticShards=new Set();room.shardSolids=new Map();room.fractureEvents=new Map();room.nextShard=1000000;}
export function chipCell(room,c,point,radius,energy,by=0,direction=[0,0,0],all=false,volume=null){
 ...
 c.skin.hp=Math.min(c.skin.hp,c.skin.maxHp*fractureStrength(c,c.skin));room.dirtyBuildings.add(c.building);
 refreshFineColliders(room,c);let event=room.fractureEvents.get(c.id);if(event)event.parts=c.skin.parts;else{event={type:'fracture',cell:c.id,parts:c.skin.parts};room.fractureEvents.set(c.id,event);room.event(event);}
...
export function spawnShards(room,c,pieces,direction=[0,0,0]){
 ...
 for(const p of pieces){const key=p.material+':'+p.p.map(n=>Math.floor(n/1.05)).join(',');if(!clusters.has(key))clusters.set(key,[]);clusters.get(key).push(p);}
...
function makeBallistic(room,e){
 ...
 const ray=room.world.castRay(new RAPIER.Ray(v(x,Math.max(.2,e.p[1]-.5),z),v(0,-1,0)),Math.max(1,e.p[1]),true,undefined,group(C.COLLISION.PLAYER,C.COLLISION.WORLD));
 e.ground=ray?Math.max(.05,e.p[1]-.5-(ray.timeOfImpact??ray.toi)):.05;e.ground+=e.half[1];
 e.duration=Math.max(.08,(e.velocity[1]+Math.sqrt(e.velocity[1]**2+48*Math.max(0,e.p[1]-e.ground)))/24);room.ballisticShards.add(e.id);
}
...
export function settleShard(room,e){
 ...
 e.settled=true;e.ballistic=false;room.activeShards.delete(e.id);room.ballisticShards.delete(e.id);
...
export function updateShards(room){
 for(const id of room.activeShards){const e=room.shards.get(id);if(e.body.isSleeping())settleShard(room,e);else if(room.time-e.born>8||e.body.translation().y<-.5){Object.assign(e,bodyPose(e.body));e.velocity=arr(e.body.linvel());removeBody(room,e.body);e.body=null;room.activeShards.delete(id);makeBallistic(room,e);room.event(shardMeta(e));}}
 for(const id of room.ballisticShards){const e=room.shards.get(id);Object.assign(e,shardBallistic(e,room.time));if(room.time-e.born>=e.duration)settleShard(room,e);}
}
export function restoreShards(room,meta){const e={...meta,body:null};room.shards.set(e.id,e);room.nextShard=Math.max(room.nextShard,e.id+1);if(e.settled)attachRestingShard(room,e);else makeBallistic(room,e);}
export function removeShards(room,id){const e=room.shards.get(id);if(e?.body)removeBody(room,e.body);if(room.shardSolids.has(id))removeBody(room,room.shardSolids.get(id));room.shardSolids.delete(id);room.activeShards.delete(id);room.ballisticShards.delete(id);room.shards.delete(id);}
