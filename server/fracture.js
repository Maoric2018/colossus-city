...
export function chipCell(room,c,point,radius,energy,by=0,direction=[0,0,0],all=false,volume=null){
 ...
 for(const p of unsupportedWallPieces(c,gone)){gone.add(p.id);chosen.push(p);delete damage[p.id];}
...
function makeBallistic(room,e){
 ...
 e.ground=supportingHeight(room,[x,e.p[1]-.04,z],e.half)+e.half[1];
...
export function updateShards(room){
 for(const id of room.activeShards){const e=room.shards.get(id);if(e.body.isSleeping()){Object.assign(e,bodyPose(e.body));if(e.p[1]-verticalHalf(e)<=supportingHeight(room,e.p,e.half)+.16)settleShard(room,e);else resumeFall(room,e);}else if(room.time-e.born>8||e.body.translation().y<-.5)resumeFall(room,e);}
 for(const id of room.ballisticShards){const e=room.shards.get(id);Object.assign(e,shardBallistic(e,room.time));if(room.time-e.born>=e.duration){const ground=supportingHeight(room,e.p,e.half)+verticalHalf(e);if(e.p[1]>ground+.16)resumeFall(room,e);else{e.p[1]=ground;settleShard(room,e);}}}
 if(!room.supportChecks.length)room.supportChecks=[...room.restingShards];
 for(let i=0;i<24&&room.supportChecks.length;i++){const id=room.supportChecks.pop(),e=room.shards.get(id);if(e?.settled&&e.p[1]-verticalHalf(e)>supportingHeight(room,e.p,e.half)+.16)resumeFall(room,e);}
}
function resumeFall(room,e){
 ...
 if(room.shardSolids.has(e.id))removeBody(room,room.shardSolids.get(e.id));room.shardSolids.delete(e.id);room.restingShards.delete(e.id);room.activeShards.delete(e.id);e.settled=false;makeBallistic(room,e);room.event(shardMeta(e));
}
