...
Generic buildings use window surrounds that match their facade textures, one main entrance, appropriate balconies and a rear fire escape on brick walkups. Rooftop services occupy one small area per building; repeated machinery, random satellite dishes, window grilles and overlapping decorative frames are removed. Interior structure remains visible after destruction, and landmarks retain their custom architectural kits. Downloaded photographic textures and compatible roof models are reused. Up to 49 detailed generated blocks surround the camera, with no simplified distant building ring. Buildings stay clear through 110 m, then fade steeply into the sky by 150 m on both Quest and desktop.
...
The Chrysler Building appears west of the central plaza and naturally in approximately one in 29 generated blocks. The temporary spawn shortcut has been removed from the player menu.
...
Robot missiles track visible raiders inside a 37° forward cone, up to 95 m away, turning at up to 69°/second. They lead moving targets by up to 0.45 seconds to catch crossing raiders. They ignore protected players and targets behind solid cover. The server sends curved-path corrections at 20 Hz; aiming and dodging still matter.
