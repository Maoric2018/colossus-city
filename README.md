...
Integrity combines support connections and load redistribution. Frame strength scales with bay width, building height, floor and landmark reinforcement; remaining connections have 3× load capacity and can bridge six bays. Severe overloads creak for 2.2–2.8 seconds before propagating. **Walking stops at a wall and can chip at most 5% of a contacted frame; it cannot grind a building down.** A deliberately moving hand crushes each wall in its path immediately; a wall-only hit leaves the frame unharmed. Touching a column or slab releases that structural bay. The same sweep continues through the new holes, including the far wall, with matching local Quest prediction. Holding still, walking and joystick turning do not become hand attacks. Local pieces break easily while surviving connections hold the larger building together; removing every support still causes a collapse.
