...
| Player | Controls |
| --- | --- |
| Laptop raider | WASD move relative to view; mouse aim; hold Space for jetpack ascent; hold Shift to soar, release to hover; E + direction dodges; C descend; hold left mouse to fire; V first/third-person view (first person by default); Q quality; Escape release pointer. |
...
**Live views:** on a laptop in the same room, press **Escape → Live Views** to open a spectator tab, or enter the room code and select **Spectate**. Every human feed uses direct WebRTC video, including the colossus’s actual left-eye view and in-world HUD. Capture targets 30 fps on desktop and 24 fps in VR at 640 × 400, preserving aspect ratio. These are capture ceilings; rendering speed and network conditions determine the delivered rate. Cards show delivered frame rate and estimated capture-to-display delay when the browser supplies it. AI drone views are labeled simulated and update locally without video encoding. Select any feed to enlarge it. Free Camera or closing the panel releases the streams; players see an indicator while watched.

For the demo, keep each player’s game visible on their device, put the Quest and spectator laptop on the same reliable Wi-Fi, and use one spectator panel. USB forwarding carries the game connection, **not WebRTC video**; even with the USB setup, the headset and laptop need a working media network path. Each additional spectator creates another video connection per player, capped at about 1.2 Mb/s per feed. The panel avoids rendering an unseen full-screen city behind the feeds. XR still needs one extra low-resolution eye render per captured frame.

If direct video cannot connect within eight seconds, that feed switches to a clearly labeled **NETWORK FALLBACK** with JPEG previews up to 15 fps. Only one image can be in flight per feed, and old queued images are replaced by the newest one. This fallback can be less smooth; close/reopen Live Views to retry video. Headset system overlays, passthrough, browser menus and audio are not shared.

Across different networks or restrictive Wi-Fi, configure a TURN relay for dependable video connectivity. The server accepts `VIEW_ICE_SERVERS` as a JSON array of standard ICE server configurations (`urls`, and optionally `username` / `credential`); the default is Google's public STUN server. TURN hosting and credentials are not bundled or deployed. ICE configuration is delivered to authenticated room participants, so use scoped, short-lived TURN credentials for a public deployment. Test the actual demo network and physical headset beforehand; local/emulated measurements do not establish headset performance.
