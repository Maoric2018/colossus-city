...
test('collision cells are genuinely hollow and openings appear when the solid skin breaks', () => {
 for(const c of cells){ const shapes = cellColliders({...c,roofAsset:null,spire:0,architecture:'urban',chryslerCrown:false}), volume = shapes.reduce((s, x) => s + 8 * x[3] * x[4] * x[5], 0), envelope = c.size.reduce((a, b) => a * b, 1); assert.ok(volume < envelope * .35); assert.ok(shapes.length >= 5); }
...
test('compound shapes are finite and stay inside each bay', () => { for(const c of cells) for(const a of cellColliders({...c,roofAsset:null,spire:0,architecture:'urban',chryslerCrown:false})){ assert.ok(a.every(Number.isFinite)); for(let k = 0; k < 3; k++){ assert.ok(a[k + 3] > 0); assert.ok(Math.abs(a[k]) + a[k + 3] <= c.size[k] / 2 + 1e-6); } } });
