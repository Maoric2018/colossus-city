...
test('hostile input cannot introduce nonfinite axes or unbounded speeds', () => { const i = sanitizeInput({yaw:0, pitch:0, x:Infinity, z:-Infinity, up:500, seq:Infinity, rocket:Infinity}); assert.equal(i.x, 1); assert.equal(i.z, -1); assert.equal(i.up, 1); assert.ok(Number.isFinite(i.seq)); assert.ok(Number.isFinite(i.rocket)); assert.equal(sanitizeInput({yaw:NaN, pitch:0}), null); });
