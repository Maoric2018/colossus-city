...
test('two clients share one authoritative match and observe the same movement',{timeout:15000},async()=>{
 const server=spawn(process.execPath,['server/index.js'],{env:{...process.env,PORT:String(port)},stdio:['ignore','pipe','pipe']});let output='';server.stderr.on('data',d=>output+=d);let a,b;
 try{
 ...
 finally{a?.ws.close();b?.ws.close();server.kill('SIGTERM');}
});
