...
test('two clients share one authoritative match and observe the same movement',{timeout:15000},async()=>{
 ...
 try{
  ...
  a=await connect({create:true,role:'boss',name:'Giant',eventFormat:1});b=await connect({room:a.welcome.room,role:'raider',name:'Raider'});
  ...
  a.ws.send(JSON.stringify({type:'input',missile:true,yaw:0,pitch:0,seq:1}));await sleep(120);
  const packed=a.messages.find(m=>m.type==='events'&&m.events.some(e=>e.type==='missile')),legacy=b.messages.find(m=>m.type==='events'&&m.events.some(e=>e.type==='missile'));assert.ok(packed&&legacy,'both protocol generations receive reliable events');assert.equal(packed.eventFormat,1);assert.equal(legacy.eventFormat,undefined);assert.deepEqual(unpackEvents(packed.events),legacy.events);
