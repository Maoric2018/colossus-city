...
test('soaring opens both sides of a building, keeps speed and health, and replicates persistent rubble',()=>{
 const {r,client,p,bounds}=setup();try{
  for(let i=0;i<45;i++)tick(r,client,{soar:true});
  ...
  const broken=r.cellsByBuilding[5].filter(c=>r.detached.has(c.id));assert.equal(broken.length,2,'only the two directly struck wall bays break');
  for(const c of broken){assert.equal(c.skin.glass,0);assert.equal(c.skin.facade,0);assert.equal(c.lastHitBy,p.id);assert.ok(r.debris.has(c.entity)||r.settled.has(c.entity));}
  const events=r.drainEvents();assert.ok(events.some(e=>e.type==='soar-breach'&&e.player===p.id));assert.ok(events.some(e=>e.type==='skin'));
  const late=r.welcome({id:99,role:'spectator'});for(const c of broken){assert.ok(late.entities.some(e=>e.cells.includes(c.id)));assert.ok(late.skins.some(s=>s[0]===c.id&&s[1]===0&&s[2]===0));}
...
test('holding soar against a wall from rest can start a breach',()=>{
 const {r,client,p,bounds,cell}=setup();try{
  ...
  for(let i=0;i<65;i++)tick(r,client,{soar:true});assert.ok(p.body&&p.body.translation().z<bounds[2]-2);assert.equal(p.hp,100);assert.ok(r.detached.size>0);
 }finally{r.dispose();}
});
