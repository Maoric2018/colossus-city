...
test('cars, lamps and signboards have world collision and stop raider movement',()=>{
 const r=new Room('ABC123');try{const car=r.props.find(p=>p.kind==='car'),box=car.boxes[0],origin=add(vec(car.position),rotateYaw(v(0,box[1],box[5]+3),car.yaw)),direction=rotateYaw(v(0,0,-1),car.yaw);r.world.step();const contact=hit(r,origin,direction,5);assert.ok(contact);assert.equal(r.colliderTags.get(contact.collider.handle)?.prop,car.id);
...
test('laser hitting a car reports a surface impact without damaging the giant',()=>{
 const r=new Room('ABC123');try{const car=r.props.find(p=>p.kind==='car'),a=car.boxes[0],origin=add(vec(car.position),rotateYaw(v(0,a[1]-.5,a[5]+3),car.yaw)),c=r.attach(ws,'raider','pilot'),p=r.players.get(c.id);r.spawn(p,origin);r.world.step();p.input.yaw=car.yaw;p.input.pitch=0;r.shoot(p);const shot=r.drainEvents().find(e=>e.type==='shot');assert.equal(shot.hit,false);assert.equal(shot.impact,true);assert.ok(Math.hypot(...shot.normal)>.99);assert.equal(r.bossHP,C.BOSS_HP);
 }finally{r.dispose();}
});
