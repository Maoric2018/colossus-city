...
import {NEW_BUILDING_STYLES,WORLD_BUILDING_STYLES} from '../shared/city/catalog.js';
const worldOnly=process.argv.includes('--world'),styles=worldOnly?WORLD_BUILDING_STYLES:NEW_BUILDING_STYLES,reportName=worldOnly?'world-landmarks':'catalog';
...
try{
 ...
 for(const style of styles){
  const result=await page.evaluate(async id=>{
   ...
   const height=Math.max(...cells.map(c=>c.p[1]+c.size[1]/2))+10,d=Math.max(height*1.3,27),camera=new T.PerspectiveCamera(43,1,.1,1000);camera.position.set(d*(STYLE_BY_ID.get(id).world?.3:.76),height*.55+5,d);camera.lookAt(0,height*.46,0);camera.updateMatrixWorld(true);
   ...
   const {roofSites}=await import('/src/render/catalog-lod.js'),{catalogRoofTypes}=await import('/shared/city/catalog-components.js');
   const rounded=a=>a.map(n=>Math.round(n*1e6)/1e6),roofKey=c=>JSON.stringify([c.floor,c.ix,c.iz,rounded(c.size),rounded(c.p),catalogRoofTypes(c)]),roofMatch=JSON.stringify(roofSites(b).map(roofKey).sort())===JSON.stringify(cells.filter(c=>c.roof).map(roofKey).sort());
 ...
 const cards=await Promise.all(styles.map(async s=>`<figure><img src="data:image/png;base64,${(await readFile(`artifacts/catalog/${s.id}.png`)).toString('base64')}"><figcaption>${s.name}${s.city?' · '+s.city:''}</figcaption></figure>`));
 await page.setViewportSize({width:1600,height:900});await page.setContent(`<style>body{margin:0;background:#17232b;color:#fff;font:16px Arial;display:grid;grid-template-columns:repeat(${worldOnly?6:8},1fr)}figure{margin:0;padding:5px}img{width:100%;display:block}figcaption{height:44px;padding:5px}</style>${cards.join('')}`);await page.screenshot({path:`artifacts/${reportName}-contact-sheet.png`,fullPage:true});
 assert.deepEqual(errors,[]);const report={result:'PASS',models,errors};await writeFile(`artifacts/${reportName}-report.json`,JSON.stringify(report,null,2));console.log(JSON.stringify({result:report.result,models:models.length,minTypes:Math.min(...models.map(m=>m.registered)),maxTypes:Math.max(...models.map(m=>m.registered)),errors}));
