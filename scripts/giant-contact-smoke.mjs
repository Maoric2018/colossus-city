...
try{
 ...
 // Exercise immediate local crushing against the real city and authoritative server
 ...
 await quest.evaluate(()=>{window.contactEvents=[];const net=window.__COLOSSUS.net,receive=net.onMessage;net.onMessage=m=>{if(m.type==='events')contactEvents.push(...m.events.filter(e=>e.type==='strike'||e.type==='fracture'||e.type==='shards'));receive(m);};});
 for(let i=0;i<32;i++){await quest.evaluate(()=>questDevice.controllers.right.position.x-=.005);await quest.waitForTimeout(45);}
 await quest.waitForFunction(()=>contactEvents.some(e=>e.type==='strike'&&e.cell));
 // Hold beyond the prediction grace: accepted holes must stay open without a snap-back.
 await quest.waitForTimeout(500);
 await quest.waitForFunction(()=>{const {giant,xr,net}=window.__COLOSSUS;return Math.abs(net.latest.right[0]-xr.local.right[0])<.03&&Math.abs(giant.arms[1].fist.position.x-xr.local.right[0])<.03;});
 const tracked=await quest.evaluate(()=>{const {giant,xr,net,city}=window.__COLOSSUS;return {raw:xr.local.right,visible:giant.arms[1].fist.position.toArray(),server:net.latest.right,eyes:window.__COLOSSUS.renderer.xr.getCamera().cameras.length,contactEvents,confirmed:contactEvents.filter(e=>e.type==='strike'&&e.cell).every(e=>e.broke?city.detached.has(e.cell):city.skins.get(e.cell).parts?.length>0)};});
 console.log('Quest hand crushing:',{...tracked,contactEvents:tracked.contactEvents.length});assert.equal(tracked.eyes,2);assert.ok(tracked.contactEvents.some(e=>e.type==='strike'&&e.cell),'The hand must receive an authoritative strike');assert.ok(tracked.confirmed,'The actual contacted surfaces must open on the renderer');assert.ok(Math.abs(tracked.visible[0]-tracked.raw[0])<.03,'The hand follows the controller through its new hole');assert.ok(Math.abs(tracked.visible[0]-tracked.server[0])<.03,'Prediction must agree with authority after its grace expires');assert.deepEqual(errors,[]);
 await quest.screenshot({path:'artifacts/quest-hand-contact.png'});
 // A real late-joining browser receives the same piece identities and renders
 // the holes without relying on any earlier strike or transient effect.
 const roomCode=await quest.evaluate(()=>window.__COLOSSUS.net.room),watch=await browser.newPage();watch.setDefaultTimeout(60000);watch.on('pageerror',e=>errors.push(e.message));
 await watch.goto(url+'/?quality=quest&room='+roomCode+'&spectator=1');await watch.waitForFunction(()=>window.__COLOSSUS.role==='spectator'&&window.__COLOSSUS.city.fine.cells.size>0);
 const ids=[...new Set(tracked.contactEvents.filter(e=>e.type==='fracture').map(e=>e.cell))];
 const readFine=ids=>{const g=window.__COLOSSUS;return ids.map(id=>({id,parts:g.city.skins.get(id).parts,shards:[...g.city.fine.shards].filter(([,e])=>e.c.id===id).map(([id])=>id).sort((a,b)=>a-b)}));};
 const pilotDamage=await quest.evaluate(readFine,ids),spectatorDamage=await watch.evaluate(readFine,ids);assert.ok(ids.length>0);assert.deepEqual(spectatorDamage,pilotDamage);report.lateSpectator={cells:ids.length,shards:spectatorDamage.reduce((n,e)=>n+e.shards.length,0),identical:true};await watch.close();
 report.tracked=tracked;await writeFile('artifacts/giant-contact-report.json',JSON.stringify(report,null,2));await quest.evaluate(()=>window.__COLOSSUS.renderer.xr.getSession().end());
