...
try{
 ...
 await context.addInitScript({content:await readFile('node_modules/iwer/build/iwer.js','utf8')+`\nwindow.questDevice=new IWER.XRDevice(IWER.metaQuest2,{stereoEnabled:true});questDevice.installRuntime({forceInstall:true});questDevice.position.set(0,1.7,0);questDevice.controllers.left.position.set(-.4,1.2,-.3);questDevice.controllers.right.position.set(1,1,-22/14);`});
 ...
 await quest.waitForFunction(()=>{const {giant,xr,net}=window.__COLOSSUS;return giant.arms[1].contacts.length&&xr.local.right[0]-net.latest.right[0]>.025;});
 ...
 console.log('Quest wall contact:',tracked);assert.equal(tracked.eyes,2);assert.ok(tracked.raw[0]-tracked.visible[0]>.025);assert.ok(Math.abs(tracked.visible[0]-tracked.server[0])<.03,'Local Quest contact must match authoritative wall contact without waiting for interpolation');
