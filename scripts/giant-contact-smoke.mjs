...
try{
 ...
 await context.addInitScript({content:await readFile('node_modules/iwer/build/iwer.js','utf8')+`\nwindow.questDevice=new IWER.XRDevice(IWER.metaQuest2,{stereoEnabled:true});questDevice.installRuntime({forceInstall:true});questDevice.position.set(-12/14,1.7,0);questDevice.controllers.left.position.set(-12/14-.4,1.2,-.3);questDevice.controllers.right.position.set(-55/14,1,0);`});
 ...
 await quest.waitForFunction(()=>{const {giant,xr,net}=window.__COLOSSUS;return giant.arms[1].contacts.length&&net.latest.right[0]-xr.local.right[0]>.025;});
 ...
 const tracked=await quest.evaluate(()=>{const {giant,xr,net}=window.__COLOSSUS;return {raw:xr.local.right,visible:giant.arms[1].fist.position.toArray(),server:net.latest.right,eyes:window.__COLOSSUS.renderer.xr.getCamera().cameras.length};});
 console.log('Quest wall contact:',tracked);assert.equal(tracked.eyes,2);assert.ok(tracked.visible[0]-tracked.raw[0]>.025);assert.ok(Math.abs(tracked.visible[0]-tracked.server[0])<.03,'Local Quest contact must match authoritative wall contact without waiting for interpolation');
