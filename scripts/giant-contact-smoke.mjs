...
try{
 ...
 await quest.evaluate(()=>{window.contactEvents=[];const net=window.__COLOSSUS.net,receive=net.onMessage;net.onMessage=m=>{if(m.type==='events')contactEvents.push(...m.events.filter(e=>e.type==='strike'||e.type==='skin'||e.type==='debris'));receive(m);};});
