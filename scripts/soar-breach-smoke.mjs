...
try{
 ...
 await page.evaluate(async ({initial,firstPerson})=>{
  ...
  window.breachPreview={pilot,handler,cameraRig,initial,latest:initial,time:0,comparisons:[],bluePulse:false,boomSeen:false,breachSeen:false,firstPerson,minimumClearance:Infinity};
 },{initial,firstPerson});
 for(let i=0;i<timeline.length+6;i++){
  await page.evaluate(({frame,delivered})=>{
   ...
   b.pilot.update(p,true,b.firstPerson,1/60,b.time);c.city.update(1/60);c.fx.update(1/60);b.bluePulse||=c.flightFX.pulse>0;b.boomSeen||=c.fx.booms.length>0&&c.flightFX.boom>0;b.breachSeen||=c.fx.booms.some(w=>w.breach)&&c.fx.dust.items.length>0&&c.fx.sparks.items.length>0;c.flightFX.update(1/60,p,true);
 ...
 const result=await page.evaluate(()=>{const c=window.__COLOSSUS,b=window.breachPreview;return {position:c.prediction.position(),server:b.latest.p,broken:[...c.city.fine.cells.keys()],fragments:c.city.fine.shards.size,pulse:b.bluePulse,sonicBoom:b.boomSeen,worldBreach:b.breachSeen,speedEffects:c.flightFX.strength,minimumSpeed:Math.min(...b.comparisons.map(p=>p.speed)),backwardSteps:b.comparisons.slice(1).filter((p,i)=>p.pos[2]>b.comparisons[i].pos[2]+.2).length,firstPerson:b.firstPerson,minimumClearance:b.minimumClearance,atlasLoaded:c.flightFX.uniforms.speedMap.value.image.width===3102&&c.fx.sonicBursts.mesh.material.uniforms.spriteMap.value.image.width===2556,atlasFrame:c.fx.sonicBursts.frames.getX(0),failedAssets:c.assetStatus.failed};});
 assert.ok(result.position.z<bounds[2]-2);assert.ok(result.broken.length>=2&&result.fragments>0&&result.pulse);assert.ok(result.minimumSpeed>25);assert.ok(result.sonicBoom&&result.speedEffects>.6);assert.ok(result.worldBreach);assert.equal(result.backwardSteps,0);assert.ok(result.atlasLoaded);assert.ok(result.minimumClearance>1,'The camera never fills with a wall while breaching');assert.deepEqual(result.failedAssets,[]);assert.deepEqual(errors,[]);
 results.push(result);
 }
 await page.close();
