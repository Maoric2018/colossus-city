...
try{
 ...
 const props=await page.evaluate(async()=>{const T=await import('three'),{city}=window.__COLOSSUS,car=city.props.find(p=>p.kind==='car'&&Math.hypot(p.position[0],p.position[2])<45),box=car.boxes[0],turn=new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),car.yaw),origin=new T.Vector3(0,box[1],box[5]+3).applyQuaternion(turn).add(new T.Vector3(...car.position)),direction=new T.Vector3(0,0,-1).applyQuaternion(turn),distance=city.rayDistance(origin,direction,5,.25);return {distance};});assert.ok(Math.abs(props.distance-2.75)<.01,'Shoulder camera must stop at the padded car surface');
