...
try{
 ...
 browser=await chromium.launch({headless:process.env.HEADLESS==='1',channel:'chromium',args:['--enable-webgl','--ignore-gpu-blocklist','--disable-background-timer-throttling','--disable-renderer-backgrounding','--disable-backgrounding-occluded-windows']});
 ...
 for(const [name,position,target]of tours){
  const sample=await page.evaluate(async({position,target})=>{
   ...
   for(let i=0;i<80;i++){renderer.render(scene,camera);await new Promise(r=>requestAnimationFrame(r));if(i>=12&&!city.stream.pending.length)break;}
