...
try{
 ...
 await page.evaluate(async()=>{
  ...
  window.sampleArmor=()=>{const {giant,renderer,xr}=window.__COLOSSUS,eye=renderer.xr.getCamera().cameras[0];return {turn:xr.turn,glow:giant.coreGlow.visible,parts:giant.arms.flatMap((arm,i)=>[arm.upper.armor,arm.lower.armor,arm.fist.children[0]].map((o,j)=>{const m=new T.Matrix4().multiplyMatrices(eye.matrixWorldInverse,o.matrixWorld),position=new T.Vector3(),orientation=new T.Quaternion(),scale=new T.Vector3();m.decompose(position,orientation,scale);return {name:`${i?'right':'left'}-${['upper','forearm','fist'][j]}`,position:position.toArray(),orientation:orientation.toArray()};}))};};
 });
 ...
 const report={sweptYaw,metrics,glow,browserErrors:errors};console.log(JSON.stringify(report,null,2));await writeFile('artifacts/xr-view-report.json',JSON.stringify(report,null,2));
 ...
 console.log('PASS: visible hands and forearms stay aligned through full joystick yaw; no self-reactor halo');
