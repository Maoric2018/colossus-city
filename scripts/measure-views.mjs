...
export async function measureViews(publishers,observer){
 ...
 for(const page of publishers){const capture=await page.evaluate(async()=>{window.restoreVideoStamp();const {views,net}=window.__COLOSSUS;return {id:net.id,captureFps:window.captureMeasurement.frames*1000/(performance.now()-window.captureMeasurement.start),hidden:document.hidden,outbound:await Promise.all([...views.stream.peers.values()].map(async p=>[...await p.pc.getStats()].map(([,s])=>s).filter(s=>s.type==='outbound-rtp').map(s=>({framesSent:s.framesSent,framesEncoded:s.framesEncoded,framesPerSecond:s.framesPerSecond,qualityLimitationReason:s.qualityLimitationReason,totalEncodeTime:s.totalEncodeTime}))))};});Object.assign(measured.find(m=>m.id===capture.id),capture);}
 return measured;
}
