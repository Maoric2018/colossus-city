...
try{
 ...
 const xr=await quest.evaluate(()=>{const g=window.__COLOSSUS;return {eyes:g.renderer.xr.getCamera().cameras.length,slabs:g.city.stream.ground.sidewalks.count};});assert.equal(xr.eyes,2);assert.equal(xr.slabs,49);await quest.screenshot({path:'artifacts/street-quest-stereo.png'});
