...
export function makeEventHandler({city, fx, audio, hud, shake, xr, missiles, flightFX, giant, addRag, rags, listenerPosition}){
 ...
 return function handle(e){
  switch(e.type){
   ...
   case 'gianthit': fx.impact(e.p, e.power * 1.2, 'steel'); audio.play('gianthit', {p:e.p, power:e.power}); if(state.role === 'boss'){ xr.flash(e.power); xr.haptic(Math.min(1, .4 + e.power), 180); shake.add(.5); hud.feed(`BREACHED · -${e.damage}`, 'warn'); } return;
