...
export class HUD {
 ...
 scoreboard(players, winner){
  const rows = players.slice().sort((a, b) => b.score - a.score).map(p => `<tr><td>${p.bot ? '◇' : '›'} ${p.name}${p.id === state.localId ? ' / YOU' : ''}</td><td>${p.kills}</td><td>${p.damage}</td><td>${p.score}</td></tr>`).join('');
