...
export class HUD {
 ...
 refresh(s, now, {net, renderer, input}){
  ...
  if(s.phase && !state.previousPhase && !renderer.renderer.xr.isPresenting && state.role !== 'spectator'){ document.exitPointerLock?.(); this.showOverlay(s.phase === 1 ? 'THE COLOSSUS HAS FALLEN.' : 'THE COLOSSUS SURVIVES.', `${s.kills} raiders down. ${Math.round(s.damage)}% structural damage, ${s.towersDown} towers down. A new round starts automatically after 20 seconds.`, {renderer:renderer.renderer, net}); }
