...
export class Input {
 ...
 bind(){
  ...
  window.addEventListener('mousedown', e => {
   ...
   if(e.button === 2){ if(state.role === 'boss') this.missileFiring = true; else if(state.role === 'raider') this.beginHeavy(); }
  });
  window.addEventListener('mouseup', e => {
   if(e.button === 0) this.firing = false;
   if(e.button === 2){ this.missileFiring = false; this.releaseHeavy(state.paused); }
  });
 }
 beginHeavy(){
  if(this.chargeStart || performance.now() < this.heavyReadyAt) return false;
  this.chargeStart = performance.now(); this.hooks.onChargeStart?.(); return true;
 }
 releaseHeavy(cancelled = false){
  if(!this.chargeStart) return false;
  const charged = this.charge >= 1; this.chargeStart = 0;
  if(cancelled) return false;
  if(charged) return this.fireHeavy();
  this.hooks.onChargeCancel?.(); return false;
 }
