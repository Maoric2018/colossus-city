...
export class Input {
 ...
 reset(){ this.keys.clear(); this.firing = false; this.missileFiring = false; this.chargeStart = 0; }
 ...
 held(){
  ...
  return {x:(k.has('KeyD') ? 1 : 0) - (k.has('KeyA') ? 1 : 0), z:(k.has('KeyS') ? 1 : 0) - (k.has('KeyW') ? 1 : 0), up:(k.has('Space') ? 1 : 0) - (k.has('KeyC') ? 1 : 0),
   boost:k.has('ShiftLeft') || k.has('ShiftRight'), fire:this.firing, soar:this.soar, dodge:this.dodgeSeq, heavy:this.heavySeq, missile:this.missileFiring || k.has('KeyR'), yaw:this.yaw, pitch:this.pitch};
 }
 ...
 idle(){ return {x:0, z:0, up:0, boost:false, fire:false, soar:this.soar, dodge:this.dodgeSeq, heavy:this.heavySeq, missile:false, yaw:this.yaw, pitch:this.pitch}; }
}
