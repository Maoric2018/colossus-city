...
export class CameraRig {
 ...
 update(dt, s, {input, net}){
  ...
  if(state.role === 'spectator'){
   ...
   desired.set(px, py + .67, pz);
   if(!state.firstPerson){ back.set(0, 1.1, 4.4).applyAxisAngle(up, yaw); const distance = back.length(); back.normalize(); const safe = this.city.rayDistance(desired, back, distance, .25); desired.addScaledVector(back, Math.max(0, safe - .18)); }
