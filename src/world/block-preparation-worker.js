...
self.onmessage=({data:{id,env,interiors,details=true}})=>{
 try{
  const key=JSON.stringify([env.key,env.cellBase,env.seed,env.buildings,interiors,details]);let entry=cache.get(key),cells;
  if(entry){cache.delete(key);cache.set(key,entry);cells=JSON.parse(entry.json);}
  else{cells=generateCells(env);for(const c of cells)c.preparedComponents=details?componentPlacements(c,{interiors}):[];const json=JSON.stringify(cells);entry={json,bytes:json.length*2};
