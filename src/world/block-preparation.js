...
export class BlockPreparation{
 constructor(interiors,details=true){this.interiors=interiors;this.details=details;this.entries=new Map();this.queue=[];this.serial=0;this.busy=null;
 ...
 next(){if(this.busy||!this.worker)return;while(this.queue.length){const e=this.queue.shift();if(this.entries.get(e.env.key)!==e)continue;this.busy=e;this.worker.postMessage({id:e.id,env:e.env,interiors:this.interiors,details:this.details});break;}}
