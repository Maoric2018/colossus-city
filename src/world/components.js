...
export class Components{
 constructor(buildings,cells,tier,concrete){
  ...
  const colors={stone:0xd2c9b8,steel:0x465059,concrete:0x999d99,bronze:0x8c7047,silver:0xc5cac9,chrome:0xdde7eb,marble:0xeeeae0,slate:0x29363d,crownGlass:0x162f3e};
  this.materials=Object.fromEntries(Object.entries(colors).map(([k,color])=>[k,surface(tier,{color,roughness:k==='chrome'?.28:k==='silver'?.5:.86,metalness:k==='chrome'?.75:0,map:k==='stone'||k==='concrete'?concrete:null})]));
  this.register(buildings,cells);
 }
 register(buildings,cells){
  for(const c of cells){const e=buildings.entries.get(c.id);this.cells.set(c.id,c);this.poses.set(c.id,e);this.entries.set(c.id,componentPlacements(c,{interiors:!this.tier.lambert}).map(p=>({...p,index:-1,cell:c,pose:e,signature:p.type.startsWith('wtc')||p.type.startsWith('empire')||p.type.startsWith('chrysler')})));}
 ...
 select(camera){
  ...
  for(const c of this.cells.values()){
   const e=this.poses.get(c.id),distance2=e.p.distanceToSquared(position),near=distance2<radius2,landmark=c.architecture==='wtc'||c.architecture==='empire'||c.architecture==='chrysler';
   ...
   for(const part of this.entries.get(c.id)){
    ...
    const visible=eligible&&(near||part.signature)&&(part.layer==='frame'||!!(mask&(1<<part.side)));
