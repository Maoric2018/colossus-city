...
export class Components{
 ...
 register(buildings,cells){
  for(const c of cells){const e=buildings.entries.get(c.id);this.cells.set(c.id,c);this.poses.set(c.id,e);this.entries.set(c.id,componentPlacements(c,{interiors:!this.tier.lambert}).map(p=>({...p,index:-1,cell:c,pose:e,signature:p.type.startsWith('wtc')||p.type.startsWith('empire')})));}
 ...
 select(camera){
  ...
  for(const c of this.cells.values()){
   const e=this.poses.get(c.id),distance2=e.p.distanceToSquared(position),near=distance2<radius2,landmark=c.architecture==='wtc'||c.architecture==='empire';
