...
export class CityView {
 constructor(scene, env, {tier, quest = false} = {}){
  this.scene = scene; this.env = env; this.tier = tier; this.quest = quest; this.root = new T.Group(); scene.add(this.root);
  ...
  for(const c of this.cells){ this.cellsByBuilding[c.building].push(c); const b = this.bounds[c.building]; for(let k = 0; k < 3; k++){ b[k] = Math.min(b[k], c.p[k] - c.size[k] / 2); b[k + 3] = Math.max(b[k + 3], c.p[k] + c.size[k] / 2); } }
  this.loader = new T.TextureLoader();
  const t = env.textures, tex = (url, repeat, srgb = true) => url ? this.texture(url, repeat, srgb) : null;
  this.textures = {concrete:tex(t.concrete, 30), concreteNormal:tex(t.concreteNormal, 30, false), concreteRoughness:tex(t.concreteRoughness, 30, false), asphalt:tex(t.asphalt, 38), asphaltNormal:tex(t.asphaltNormal, 38, false), asphaltRoughness:tex(t.asphaltRoughness, 38, false)};
 ...
 makeSkyAndLights(){
  const env = this.env;
  this.scene.fog = new T.FogExp2(env.sky.fog, env.sky.fogDensity);
  const sky = new T.Mesh(new T.SphereGeometry(800, 24, 12), new T.ShaderMaterial({vertexShader:skyVertex, fragmentShader:skyFragment, uniforms:{topColor:{value:new T.Color(env.sky.top)}, horizon:{value:new T.Color(env.sky.horizon)}}, side:T.BackSide, depthWrite:false})); this.sky = sky; sky.renderOrder = -100; this.root.add(sky);
 ...
 setSkin(id, glass, facade, fx = true){
  ...
  s.glass = glass; s.facade = facade; this.buildings.setSkin(id, glass, facade); this.colliderCache.set(id, cellColliders(c, s));
 ...
 hideCells(ids){ for(const id of ids){ this.detached.add(id); this.buildings.setCell(id, null, null, true); } }
 addDebris(e){ const entry = {cells:e.cells, origin:new T.Vector3(...e.origin), material:e.material, pos:new T.Vector3(), rot:new T.Quaternion(), radius:0}; for(const id of e.cells){ this.detached.add(id); entry.radius = Math.max(entry.radius, new T.Vector3(...this.byId.get(id).p).distanceTo(entry.origin) + 5); } this.moving.set(e.id, entry); this.poseDebris(e.id, e.p, e.q); }
 poseDebris(id, p, q){
 ...
 removeDebris(id){ const e = this.moving.get(id); if(!e) return; for(const c of e.cells) this.buildings.setCell(c, null, null, true); this.moving.delete(id); }
 ...
 crumble(ev){
  ...
  for(const cId of ev.cells){
   ...
   this.detached.add(cId); this.buildings.setCell(cId, null, null, true);
  }
  if(ev.id) this.moving.delete(ev.id);
 }
 reset(){ this.moving.clear(); this.detached.clear(); for(const c of this.cells){ const s = initialSkin(c); this.skins.set(c.id, s); this.colliderCache.set(c.id, cellColliders(c)); this.buildings.setSkin(c.id, s.glass, s.facade); this.buildings.setCell(c.id, tp.set(c.p[0], c.p[1], c.p[2]), tq.identity(), false); } this.commit(); }
 ...
 rayDistance(origin, direction, maxDistance, padding = 0){
  ...
  for(const bi of this.rayBuilding(origin, direction, result)) for(const c of this.cellsByBuilding[bi]){
   ...
   tc.set(c.p[0], c.p[1], c.p[2]); th.set(c.size[0] / 2, c.size[1] / 2, c.size[2] / 2);
  ...
  for(const e of this.moving.values()){
   ...
   for(const cId of e.cells){
    ...
    th.set(c.size[0] / 2, c.size[1] / 2, c.size[2] / 2); tc.set(0, 0, 0); if(rayAABB(to, td, tc, th, result, padding) === Infinity) continue;
 ...
 overlapBox(center, half){
  for(let bi = 0; bi < this.bounds.length; bi++){
   ...
   for(const c of this.cellsByBuilding[bi]){
    if(this.detached.has(c.id) || Math.abs(center.x - c.p[0]) > half.x + c.size[0] / 2 || Math.abs(center.y - c.p[1]) > half.y + c.size[1] / 2 || Math.abs(center.z - c.p[2]) > half.z + c.size[2] / 2) continue;
