...
import {fractureRecipe,pieceAlive} from '../../shared/city/fracture.js';
import {RenderWarmup} from '../render/warmup.js';
...
export class CityView {
 constructor(scene, env, {tier = TIERS.low, quest = false, parent = null, preparedCells = null, deferred = false} = {}){
  ...
  this.initializing=true;this.root.visible=!deferred;this.initialization=this.initialize(preparedCells);
  if(!deferred)while(!this.advance(Infinity)){}
 }
 advance(deadline){
  if(!this.initializing)return true;
  do{if(this.initialization.next().done){this.initializing=false;return true;}}while(performance.now()<deadline);
  return false;
 }
 activate(){this.root.visible=true;for(const e of this.buildings.entries.values())e.pending=false;this.buildings.components.selectionDirty=true;}
 *initialize(preparedCells){
  const {parent,scene,env,tier,quest}=this;
  ...
  this.handWorld = new HandWorld(env,[]); this.props = staticProps(env);
  this.skins = new Map();this.colliderCache = new Map();
  ...
  for(let i=0;i<this.cells.length;i+=16){const cells=this.cells.slice(i,i+16);this.handWorld.addCells(cells);
   for(const c of cells){this.skins.set(c.id,initialSkin(c));const colliders=cellColliders(c);this.colliderCache.set(c.id,colliders);c.queryHalf=c.size.map(v=>v/2);for(const a of colliders)for(let k=0;k<3;k++)c.queryHalf[k]=Math.max(c.queryHalf[k],Math.abs(a[k])+a[k+3]);
    this.cellsByBuilding[c.building].push(c);const b=this.bounds[c.building];for(let k=0;k<3;k++){b[k]=Math.min(b[k],c.p[k]-c.queryHalf[k]);b[k+3]=Math.max(b[k+3],c.p[k]+c.queryHalf[k]);}
   }yield;
  }
  ...
  if(!parent)scene.onBeforeRender=(renderer,_scene,camera)=>{if(scene.userData.reuseCityVisibility)return;(this.warmup??=new RenderWarmup(renderer,this)).observe(camera);this.buildings.components.enableBatching(renderer);this.stream?.select(camera);const far=this.scene.fog.far||Infinity;this.buildings.select(camera,far,renderer.shadowMap.enabled);for(const v of this.stream?.views.values()||[])v.buildings.select(camera,far,renderer.shadowMap.enabled);this.buildings.components.select(camera);this.fine.select(camera,far);this.commit();};
  for(let i=0;i<this.cells.length;i+=32){for(const c of this.cells.slice(i,i+32)){const s=this.skins.get(c.id);this.buildings.setSkin(c.id,s.glass,s.facade);}yield;}
  ...
  if(this.root.visible)this.activate();
 }
 ...
 makeSkyAndLights(){
  const env = this.env;
  // Fade inside the streamed footprint. Phones use a much smaller neighborhood.
  if(env.infinite)installDistanceFog();
  this.scene.fog = env.infinite ? new T.Fog(env.sky.fog,this.tier.fogNear??110,this.tier.fogFar??150) : new T.FogExp2(env.sky.fog, env.sky.fogDensity);
 ...
 setFracture(id,parts){
  ...
  const pose=this.buildings.pose(id),gone=new Set(parts);
  if(parts.length&&fractureRecipe(c).pieces.every(p=>!pieceAlive(p,s,gone))){this.fine.queueCollapse(c,s,pose,this.buildings);if(this.detached.has(id))this.fine.deferHide(id);return;}
  this.fine.set(c,s,pose,this.buildings);pose.skinDirty=true;this.buildings.setCell(id,null,null,pose.hidden);this.buildings.components.selectionDirty=true;
 }
 ...
 hideCells(ids){ for(const id of ids){ if(!this.byId.has(id)){this.stream?.hideCells([id]);continue;}this.detached.add(id); this.handWorld.setCell(id,null,undefined,true);if(!this.fine.deferHide(id))this.buildings.setCell(id, null, null, true); } }
 ...
 dispose(){this.disposed=true;this.warmup?.dispose();this.initialization?.return();this.stream?.preparation.dispose();this.stream?.reset();if(this.parent)this.fine?.unregister(this.cells);else this.fine?.reset();this.buildings?.components.unregister(this.cells);if(!this.parent)this.buildings?.components.materialBatches?.dispose();this.root.traverse(o=>{if(o.isInstancedMesh)o.dispose();});for(const mesh of this.fragments?.batches.values()||[]){mesh.geometry.dispose();mesh.material.dispose();}this.root.removeFromParent();}
