...
export function streamGround(root,tier,textures,far=130){
 const radius=Math.ceil(far/STREET.block),count=(radius*2+1)**2;
 ...
 const sidewalks=new T.InstancedMesh(new T.BoxGeometry(...SIDEWALK_HALF.map(v=>v*2)),streetMaterial(tier,textures,true),count);
 ...
 const update=p=>{
  ...
  let index=0;for(let z=cz-radius;z<=cz+radius;z++)for(let x=cx-radius;x<=cx+radius;x++)sidewalks.setMatrixAt(index++,matrix.makeTranslation(x*STREET.block,STREET.curb/2,z*STREET.block));
