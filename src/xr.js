...
export class XRControl{
 constructor(renderer,camera,rig,connection,{onEnter,onExit}={}){
  this.renderer=renderer;this.camera=camera;this.rig=rig;this.net=connection;this.onEnter=onEnter;this.onExit=onExit;this.local=null;this.lastSend=0;this.snapLatch=false;this.scale=C.GIANT_SCALE;this.turn=0;this.session=null;this.hudTime=0;
 ...
 stopTracking(){
  this.resetPose();this.snapLatch=false;this.calibrateLatch=false;
 ...
 async enter(){
  ...
  try{
   ...
   this.turn=0;this.lastSend=0;this.snapLatch=false;this.calibrateLatch=false;this.trackingStopped=false;this.resetPose();this.fps=0;this.frames=0;this.fpsStart=performance.now();
 ...
 update(frame,snapshot,now){
  ...
  this.rig.position.set(latest?.bossX||0,0,latest?.bossZ||0);this.rig.rotation.y=this.turn;this.rig.updateMatrixWorld(true);
  const sources=[...this.session.inputSources],poses={};let mx=0,mz=0;
  for(const src of sources){
   const pose=src.gripSpace&&frame.getPose(src.gripSpace,ref);if(pose&&src.gamepad&&['left','right'].includes(src.handedness))poses[src.handedness]=pose.transform.position;
   const gp=src.gamepad;if(!gp)continue;const a=gp.axes,n=a.length;
   if(src.handedness==='left'){mx=a[n>=4?2:0]||0;mz=a[n>=4?3:1]||0;}
   if(src.handedness==='right'){
    const x=a[n>=4?2:0]||0;if(Math.abs(x)>.7&&!this.snapLatch){this.turn-=Math.sign(x)*Math.PI/6;this.snapLatch=true;this.resetPose();}if(Math.abs(x)<.3)this.snapLatch=false;
  ...
  const head=world(viewer.transform.position);
  ...
  const local={...(snapshot||{}),head,bossYaw:euler.y,bossX:this.rig.position.x,bossZ:this.rig.position.z,left:poses.left?world(poses.left):(this.local?.left||[head[0]-5,head[1]-7,head[2]-4]),right:poses.right?world(poses.right):(this.local?.right||[head[0]+5,head[1]-7,head[2]-4])};this.local=local;
  if(!poses.left||!poses.right)this.stopTracking();
  else if(now-this.lastSend>1000/C.INPUT_HZ){
   if(this.net.send({type:'pose',head:local.head,left:local.left,right:local.right,yaw:local.bossYaw,moveX:Math.abs(mx)>.15?mx:0,moveZ:Math.abs(mz)>.15?mz:0,reset:this.poseReset===true})){this.lastSend=now;this.poseReset=false;this.trackingStopped=false;}
  }
 ...
 paintHUD(s,tracking){
  ...
  x.font='20px monospace';x.fillStyle=tracking?'#99b7bd':'#ffbc80';x.fillText(!tracking?'CONTROLLER TRACKING LOST · HOLD STILL':s?.phase===1?'RAIDERS WIN · NEW ROUND IN 20 SECONDS':s?.phase===2?'COLOSSUS WINS · NEW ROUND IN 20 SECONDS':'LEFT STICK: MOVE   RIGHT: SNAP TURN   A: RECALIBRATE',35,204);
  x.font='18px monospace';x.fillText(`${this.fps||0} FPS · ${this.net.ping||0}ms · TARGET ${this.session?.frameRate||'AUTO'} Hz`,35,238);this.hudTexture.needsUpdate=true;
 }
