...
export class MissileView{
 ...
 update(time){
  ...
  for(const [id,m]of this.active){const age=Math.max(0,time-m.born);if(age>m.life+.3){this.active.delete(id);continue;}if(i>=C.MAX_MISSILES)break;
   const dir=new T.Vector3(...m.direction),target=new T.Vector3(...m.p).addScaledVector(dir,Math.max(0,Math.min(.1,time-m.time))*C.MISSILE_SPEED);if(!m.rendered)m.rendered=target.clone();else m.rendered.lerp(target,.65);const position=m.rendered.clone();dummy.position.copy(position);dummy.quaternion.setFromUnitVectors(up,dir);dummy.scale.setScalar(1);dummy.updateMatrix();for(const batch of this.batches)batch.setMatrixAt(i,dummy.matrix);i++;
