// 32-byte header + 60-byte boss + players(56 each) + moving bodies(32 each).
// Float32 positions/quaternions, not JSON transforms. Little-endian, versioned.
export const MAGIC=0x434f4c32;
export const KIND={CELL:0,RAGDOLL:1};
export function encodeSnapshot(s){
 const p=s.players||[], b=s.bodies||[],buffer=new ArrayBuffer(92+p.length*56+b.length*32),d=new DataView(buffer);let o=0;
...
export function decodeSnapshot(buffer){
 ...
 if(np>9||nb>1200||d.byteLength!==92+np*56+nb*32)throw Error('Invalid snapshot size');
