...
// 32-byte header + 92-byte boss + players(56 each) + moving bodies(32 each).
// Float32 positions/quaternions, not JSON transforms. Little-endian, versioned.
export const MAGIC=0x434f4c33;
export const KIND={CELL:0,RAGDOLL:1};
export function encodeSnapshot(s){
 const p=s.players||[], b=s.bodies||[],buffer=new ArrayBuffer(124+p.length*56+b.length*32),d=new DataView(buffer);let o=0;
 ...
 for(const a of p){u(a.id);u(a.flags);for(const x of a.p)f(x);for(const x of a.v)f(x);f(a.yaw);f(a.hp);f(a.fuel);f(a.seq);f(a.pitch);f(a.dodgeCooldown);}
...
export function decodeSnapshot(buffer){
 ...
 if(d.byteLength<124||d.getUint32(0,true)!==MAGIC)throw Error('Bad snapshot');
 ...
 if(np>9||nb>1200||d.byteLength!==124+np*56+nb*32)throw Error('Invalid snapshot size');
 ...
 s.players=[];s.bodies=[];
 for(let i=0;i<np;i++)s.players.push({id:u(),flags:u(),p:[f(),f(),f()],v:[f(),f(),f()],yaw:f(),hp:f(),fuel:f(),seq:f(),pitch:f(),dodgeCooldown:f()});
