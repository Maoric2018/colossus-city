...
export class HandWorld{
 constructor(env,cells){
  this.cells=new Map(cells.map(c=>[c.id,{cell:c,local:cellColliders(c,c.skin),boxes:[]}]));this.children=new Set();this.buckets=new Map();this.fixed=[box(env.center?[env.center[0],-.3,env.center[1]]:[0,-.3,0],[env.cellBase?35:220,.3,env.cellBase?35:220])];
 ...
 addCells(cells){for(const c of cells){this.cells.set(c.id,{cell:c,local:cellColliders(c,c.skin),boxes:[]});this.setCell(c.id,c.p,identity);}}
