...
export function sanitizeInput(m){
 ...
 return {x:clamp(Number(m.x)||0,-1,1),z:clamp(Number(m.z)||0,-1,1),up:clamp(Number(m.up)||0,-1,1),boost:!!m.boost,fire:!!m.fire,soar:!!m.soar,missile:!!m.missile,dodge:clamp(Math.floor(Number(m.dodge)||0),0,1e9),heavy:clamp(Math.floor(Number(m.heavy)||0),0,1e9),
