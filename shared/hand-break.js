...
export const handSurfaceKey=contact=>contact.part!=null?`${contact.cell}:part:${contact.part}`:contact.kind==='wall'?`${contact.cell}:wall:${contact.side}`:`${contact.cell}:frame`;
...
export function resolveBreakableHand(from,target,q,world,velocity=[0,0,0],open=()=>true,ignored=new Set()){
 ...
 for(let pass=0;pass<C.HAND_BREAK_LIMIT;pass++){
  ...
  for(const contact of result.contacts){
   ...
   const opened=open(contact);if(opened){
    if(opened!=='changed')ignored.add(handSurfaceKey(contact));broken.push(contact);changed=true;
   }
  }
